...
public static class ObjToStr
{
	...
	public static string Format(object obj, Style style)
	{
		...
		lock (_stringBuilder)
		{
			...
			{
				ToStringMember m = desc.Members[i];
				Type dataType = m.Member.GetDataType();
				object val = m.Member.GetValue(obj);
				_stringBuilder.Append(dataType.Name);
				_stringBuilder.Append(" ");
				_stringBuilder.Append(m.Name);
				...
				else
				{
					...
					if (vtype.HasElementType)
					{
						...
						string ecount = "?";
						_stringBuilder.AppendFormat(m.Format, val, etype, ecount);
					}
					else
					{
						_stringBuilder.AppendFormat(m.Format, val);
					}
				}
