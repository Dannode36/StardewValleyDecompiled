...
public class FarmerSprite : AnimatedSprite
{
		...
		if (timer > interval * intervalModifier)
		{
			...
			if (currentAnimationIndex > currentAnimationFrames - 1)
			{
				...
				if (endOfAnimationFunction != null)
				{
					...
					if (!(owner.CurrentTool is MeleeWeapon weapon) || (int)weapon.type != 1)
					{
						doneWithAnimation();
					}
					return;
	...
	private void checkForFootstep()
	{
		...
		if (((currentSingleAnimation >= 32 && currentSingleAnimation <= 56) || (currentSingleAnimation >= 128 && currentSingleAnimation <= 152)) && currentAnimationIndex % 4 == 0)
		{
			...
			{
				TemporaryAnimatedSprite sprite = TemporaryAnimatedSprite.GetTemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(128, 2948, 64, 64), 80f, 8, 0, new Vector2(owner_position.X + 16f + (float)Game1.random.Next(-8, 8), owner_position.Y + (float)(Game1.random.Next(-3, -1) * 4)), flicker: false, Game1.random.NextBool(), owner_position.Y / 10000f, 0.03f, Color.Khaki * 0.45f, 0.75f + (float)Game1.random.Next(-3, 4) * 0.05f, 0f, 0f, 0f);
				Game1.currentLocation.temporarySprites.Add(sprite);
				sprite = TemporaryAnimatedSprite.GetTemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(128, 2948, 64, 64), 80f, 8, 0, new Vector2(owner_position.X + 16f + (float)Game1.random.Next(-4, 4), owner_position.Y + (float)(Game1.random.Next(-3, -1) * 4)), flicker: false, Game1.random.NextBool(), owner_position.Y / 10000f, 0.03f, Color.Khaki * 0.45f, 0.55f + (float)Game1.random.Next(-3, 4) * 0.05f, 0f, 0f, 0f);
