		...
		if (Game1.IsMasterGame)
		{
			...
			default:
			{
				...
				if ((int)level > 1 && layout_random.NextDouble() < 0.5 * (double)luckMultiplier)
				{
					...
					{
						if (activeLevels[i].layoutIndex.Value >= 32)
						{
				...
				for (int i = 0; i < activeLevels.Count; i++)
				{
					if (activeLevels[i].level.Value == level.Value - 1)
					{
						valid_layouts.Remove(activeLevels[i].layoutIndex.Value);
						break;
					}
				}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					foreach (Vector2 v in Utility.getAdjacentTileLocations(tile))
					{
						if (objects.TryGetValue(v, out var obj) && obj.QualifiedItemId.StartsWith("(O)VolcanoCoalNode"))
						{
							chance += 0.2;
						}
					}
			...
			for (int x = 0; x < mapWidth; x++)
			{
				...
				else if (left_height < height)
				{
					...
					else
					{
						...
						if (height > wall_height)
						{
							SetTile(target_layer, x, y, GetTileIndex(source_x, source_y + 1 + index));
						}
						else
						{
							SetTile(target_layer, x, y, GetTileIndex(source_x + index, source_y + 1 + random_wall_variants * 2 + 1 - height - 1));
						}
