...
public class InventoryMenu : IClickableMenu
{
	...
	public List<ClickableComponent> GetBorder(BorderSide side)
	{
		...
		case BorderSide.Bottom:
		{
			...
			{
				if (i >= actualInventory.Count - row_size)
				{
	...
	public Item rightClick(int x, int y, Item toAddTo, bool playSound = true, bool onlyCheckToolAttachments = false)
	{
		foreach (ClickableComponent item in inventory)
		{
			...
			if (actualInventory[slotNumber] is Tool tool && (toAddTo == null || toAddTo is Object) && tool.canThisBeAttached((Object)toAddTo))
			{
	...
	public override void draw(SpriteBatch b, int red = -1, int green = -1, int blue = -1)
	{
		...
		if (drawSlots)
		{
			for (int i = 0; i < capacity; i++)
			{
				...
				if ((playerInventory || showGrayedOutSlots) && i >= (int)Game1.player.maxItems)
				{
				...
				if (!Game1.options.gamepadControls && i < 12 && playerInventory)
				{
					...
					b.DrawString(Game1.tinyFont, strToDraw, toDraw + new Vector2(32f - strSize.X / 2f, 0f - strSize.Y), (i == Game1.player.CurrentToolIndex) ? Color.Red : Color.DimGray);
				}
			}
			for (int i = 0; i < capacity; i++)
			{
				...
				if (actualInventory.Count > i && actualInventory[i] != null)
				{
					...
					actualInventory[i].drawInMenu(b, toDraw, (inventory.Count > i) ? inventory[i].scale : 1f, (!highlightMethod(actualInventory[i])) ? 0.25f : 1f, 0.865f, StackDrawType.Draw, Color.White, highlight);
				}
			}
			return;
		}
