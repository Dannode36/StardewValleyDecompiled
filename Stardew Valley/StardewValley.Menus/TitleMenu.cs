...
public class TitleMenu : IClickableMenu, IDisposable
{
			...
			else if (titleInPosition)
			{
				...
				if (Game1.content.GetCurrentLanguage() != LocalizedContentManager.LanguageCode.zh)
				{
					...
					else if (cornerRect.Contains(x, y) && !cornerPhaseHolding)
					{
						...
						else
						{
							...
							for (int i = 0; i < 3; i++)
							{
								tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(280 + Game1.random.Choose(8, 0), 1954, 8, 8), 1000f, 1, 99, new Vector2(width / 2 - 190 * zoom, -300 * zoom - (int)(viewportY / 3f) * zoom + 175 * zoom), flicker: false, flipped: false, 1f, 0f, Color.White, 3f, 0f, 0f, (float)Game1.random.Next(-10, 11) / 100f)
								{
				...
				if (clicked)
				{
					...
					if (clicksOnLeaf == 10)
					{
						...
						tempSprites.Add(new TemporaryAnimatedSprite("Minigames\\TitleButtons", new Rectangle(264, 464, 16, 16), new Vector2(width / 2 - 200 * zoom + 80 * zoom, -300 * zoom - (int)(viewportY / 3f) * zoom + 10 * zoom + 2), flipped: false, 0f, Color.White)
						{
							scale = zoom,
							animationLength = 8,
							interval = 80f,
						...
						tempSprites.Add(new TemporaryAnimatedSprite("Minigames\\TitleButtons", new Rectangle(136, 448, 16, 16), new Vector2(width / 2 - 200 * zoom + 80 * zoom, -300 * zoom - (int)(viewportY / 3f) * zoom + 10 * zoom), flipped: false, 0f, Color.White)
						{
							scale = zoom,
							animationLength = 8,
							interval = 50f,
						...
						tempSprites.Add(new TemporaryAnimatedSprite("Minigames\\TitleButtons", new Rectangle(200, 464, 16, 16), new Vector2(width / 2 - 200 * zoom + 178 * zoom, -300 * zoom - (int)(viewportY / 3f) * zoom + 141 * zoom + 2), flipped: false, 0f, Color.White)
						{
							scale = zoom,
							animationLength = 4,
							interval = 150f,
						...
						tempSprites.Add(new TemporaryAnimatedSprite("Minigames\\TitleButtons", new Rectangle(136, 448, 16, 16), new Vector2(width / 2 - 200 * zoom + 178 * zoom, -300 * zoom - (int)(viewportY / 3f) * zoom + 141 * zoom), flipped: false, 0f, Color.White)
						{
							scale = zoom,
							animationLength = 8,
							interval = 50f,
						...
						tempSprites.Add(new TemporaryAnimatedSprite("Minigames\\TitleButtons", new Rectangle(136, 464, 16, 16), new Vector2(width / 2 - 200 * zoom + 294 * zoom, -300 * zoom - (int)(viewportY / 3f) * zoom + 89 * zoom + 2), flipped: false, 0f, Color.White)
						{
							scale = zoom,
							animationLength = 4,
							interval = 150f,
						...
						tempSprites.Add(new TemporaryAnimatedSprite("Minigames\\TitleButtons", new Rectangle(136, 448, 16, 16), new Vector2(width / 2 - 200 * zoom + 294 * zoom, -300 * zoom - (int)(viewportY / 3f) * zoom + 89 * zoom), flipped: false, 0f, Color.White)
						{
							scale = zoom,
							animationLength = 8,
							interval = 50f,
	...
	public override void update(GameTime time)
	{
				...
				if (cornerClickParrotTimer <= 0f)
				{
					...
					for (int i = 0; i < 14; i++)
					{
						tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(355, 1199, 16, 16), new Vector2(globalXOffset + width / 2 + 220 * zoom, (float)(-300 * zoom) - viewportY / 3f * (float)zoom + (float)(60 * zoom) + (float)(Game1.random.Next(100) * zoom)), Game1.random.NextBool(), 0f, new Color(180, 180, 240))
						{
							...
							local = true,
							holdLastFrame = false,
							delayBeforeAnimationStart = 900 + i * 20
						});
					}
				}
			}
		...
		for (int i = smallClouds.Count - 1; i >= 0; i--)
		{
			smallClouds[i] -= 0.3f;
			smallClouds[i] += buttonsDX * time.ElapsedGameTime.Milliseconds / 2;
			if (smallClouds[i] < -447f)
			{
				smallClouds[i] = width;
			}
		}
		...
		for (int i = behindSignTempSprites.Count - 1; i >= 0; i--)
		{
			if (behindSignTempSprites[i].update(time))
			{
				behindSignTempSprites.RemoveAt(i);
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (!whichSubMenu.Equals("Load"))
		{
			...
			{
				b.Draw(Game1.mouseCursors, new Vector2(x * 3, -1080f - viewportY * 0.66f), new Rectangle(0, 1453, 638, 195), Color.White * (1f - (float)globalXOffset / 1200f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.8f);
			}
		}
		...
		for (int i = 0; i < smallClouds.Count; i++)
		{
			b.Draw(cloudsTexture, new Vector2(smallClouds[i], (float)(height - 900 - i * 12 * 3) - viewportY * 0.5f), (i % 3 == 0) ? new Rectangle(152, 447, 123, 55) : ((i % 3 == 1) ? new Rectangle(0, 471, 149, 66) : new Rectangle(410, 467, 63, 37)), Color.White * globalCloudAlpha, 0f, Vector2.Zero, 3f, SpriteEffects.None, 0.01f);
		}
		...
		for (int y = 0; y < height; y += 195)
		{
			...
			{
				b.Draw(Game1.mouseCursors, new Vector2(x, y) * 4f, new Rectangle(0, 1453, 638, 195), Color.White * ((float)globalXOffset / 1200f), 0f, Vector2.Zero, 4f, effect, 0.8f);
			}
