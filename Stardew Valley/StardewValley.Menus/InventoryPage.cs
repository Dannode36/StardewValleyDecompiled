...
public class InventoryPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		_horse = Game1.getCharacterFromName<Horse>(Game1.player.horseName, mustBeVillager: false);
		...
		foreach (ClickableComponent c in equipmentIcons)
		{
			...
			case "Right Ring":
				if (newItem == null || newItem is Ring)
				{
					...
					Item oldItem = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Ring)newItem, ringField));
					setHeldItem(oldItem);
			...
			case "Boots":
				...
				{
					Item oldItem = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Boots)newItem, Game1.player.boots));
					setHeldItem(oldItem);
			...
			case "Shirt":
			{
				...
				Item oldItem = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Clothing)newItem, Game1.player.shirtItem));
				setHeldItem(oldItem);
		...
		else if (checkHeldItem() && Game1.oldKBState.IsKeyDown(Keys.LeftShift))
		{
			...
			else if (checkHeldItem((Item i) => i is Trinket) && Game1.player.stats.Get("trinketSlots") != 0)
			{
				...
				for (int i = 0; i < Game1.player.trinketItems.Count; i++)
				{
					...
					{
						Game1.player.trinketItems[i] = takeHeldItem() as Trinket;
						success = true;
						break;
					}
				}
			...
			{
				int i;
				for (i = 12; i < Game1.player.Items.Count; i++)
				{
					if (Game1.player.Items[i] == null || checkHeldItem((Item item) => Game1.player.Items[i].canStackWith(item)))
					{
						if (Game1.player.CurrentToolIndex == i && checkHeldItem())
