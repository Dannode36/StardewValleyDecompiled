	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		if (!deleteConfirmationScreen)
		{
			for (int i = 0; i < slotButtons.Count; i++)
			{
				...
				if (MenuSlots[currentItemIndex + i] is SaveFileSlot { versionComparison: <0 } menu_save_slot)
				{
				...
				timerToLoad = MenuSlots[currentItemIndex + i].ActivateDelay;
				if (timerToLoad > 0)
				{
					loading = true;
				...
				{
					MenuSlots[currentItemIndex + i].Activate();
				}
				return;
			}
		}
