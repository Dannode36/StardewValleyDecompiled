		...
		if (_buildAnimationCount > 0)
		{
			...
			else
			{
				...
				for (int i = 0; i < 20; i++)
				{
					...
					int x = (int)Utility.RandomFloat((rectangle.Left - 1) * 64, 64 * rectangle.Right);
					int y = (int)Utility.RandomFloat((rectangle.Top - 1) * 64, 64 * rectangle.Bottom);
					_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90) - 64, 6, 1, new Vector2(x, y), flicker: false, Game1.random.NextBool()));
					_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90) - 64, 6, 1, new Vector2(x, y), flicker: false, Game1.random.NextBool()));
				}
			}
		}
