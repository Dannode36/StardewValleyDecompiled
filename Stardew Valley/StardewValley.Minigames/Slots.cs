...
public class Slots : IMinigame
{
	...
	public void setSlotResults(List<float> toSet)
	{
		...
		if (d < 0.4 * modifier)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				toSet[i] = ((i == whereToPutStar) ? 7 : Game1.random.Next(7));
			}
			payoutModifier = 2f;
			return;
		}
	...
	public void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			b.Draw(Game1.objectSpriteSheet, basePos + new Vector2(8f, 8 + (i + 2) * 68), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, getIconIndex(which), 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
			b.Draw(Game1.objectSpriteSheet, basePos + new Vector2(76f, 8 + (i + 2) * 68), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, getIconIndex(which), 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
			b.Draw(Game1.objectSpriteSheet, basePos + new Vector2(144f, 8 + (i + 2) * 68), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, getIconIndex(which), 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
			int payout = 0;
			switch (i)
			{
			case 0:
			...
			SpriteText.drawString(b, "x" + payout, (int)basePos.X + 192 + 16, (int)basePos.Y + (i + 2) * 68 + 24, 9999, -1, 99999, 1f, 0.88f, junimoText: false, -1, "", SpriteText.color_White);
		}
		...
		for (int i = 1; i < 8; i++)
		{
			IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(379, 357, 3, 3), (int)basePos.X - 640 - 4 * i, (int)basePos.Y - 4 * i, 1024 + 8 * i, 704 + 8 * i, Color.Red * (1f - (float)i * 0.15f), 4f, drawShadow: false);
		}
		for (int i = 0; i < 17; i++)
		{
