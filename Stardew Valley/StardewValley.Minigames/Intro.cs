...
public class Intro : IMinigame
{
	...
	public void updateRoad(GameTime time)
	{
		...
		if (roadPosition >= (float)(tileSize * 3))
		{
			...
			if (cameraCenteredOnBus && !addedSign && Game1.random.NextDouble() < 0.25)
			{
				for (int i = 0; i < 3; i++)
				{
					if (i == 0 && Game1.random.NextDouble() < 0.3)
					{
	...
	public void drawRoadArea(SpriteBatch b)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			b.Draw(Game1.mouseCursors, new Vector2(-10f + (0f - valleyPosition) / 2f + (float)(i * 639) - (float)(i * 2), 70f), new Rectangle(0, 886, 639, 148), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.08f);
			b.Draw(Game1.mouseCursors, new Vector2(0f - valleyPosition + (float)(i * 639) - (float)(i * 2), 80f), new Rectangle(0, 737, 639, 120), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.08f);
		}
		for (int i = 0; i < 8; i++)
		{
			b.Draw(treeStripTexture, new Vector2((float)(i * 256) - treePosition, 110f), new Rectangle(0, 0, 256, 64), Color.White);
		}
		for (int i = 0; i < road.Count; i++)
		{
			if (i % 3 == 0)
			{
				b.Draw(texture, new Vector2((float)(i * 16) - roadPosition, 160f), new Rectangle(0, 176, 48, 48), Color.White);
				b.Draw(texture, new Vector2((float)(i * 16 + tileSize) - roadPosition, 272f), new Rectangle(0, 64, 16, 16), Color.White);
			}
			b.Draw(texture, new Vector2((float)(i * 16) - roadPosition, 208f), new Rectangle(road[i] * 16, 240, 16, 16), Color.White);
		}
		for (int i = 0; i < roadsideObjects.Count; i++)
		{
			switch (roadsideObjects[i])
			{
			case 0:
				b.Draw(roadsideTexture, new Vector2((float)(i * 16) - roadPosition - 32f, 96f), new Rectangle(48, 0, 48, 96), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				break;
			case 1:
				b.Draw(roadsideTexture, new Vector2((float)(i * 16) - roadPosition - 32f, 96f), new Rectangle(0, 0, 48, 64), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				b.Draw(roadsideTexture, new Vector2((float)(i * 16) - roadPosition - 16f, 160f), new Rectangle(16, 64, 16, 32), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				break;
			case 2:
				b.Draw(roadsideTexture, new Vector2((float)(i * 16) - roadPosition - 32f, 176f), new Rectangle(112, 144, 16, 16), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				break;
			case 3:
				b.Draw(roadsideTexture, new Vector2((float)(i * 16) - roadPosition - 32f, 176f), new Rectangle(112, 160, 16, 16), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				break;
			case 5:
				b.Draw(texture, new Vector2((float)(i * 16) - roadPosition - 32f, 128f), new Rectangle(48, 176, 64, 64), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				break;
			}
		}
