...
public class MineCart : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		if (deathTimer <= 0f && respawnCounter > 0)
		{
			for (int i = 0; i < _entities.Count; i++)
			{
				_entities[i].OnPlayerReset();
			}
		}
		for (int i = 0; i < _entities.Count; i++)
		{
			if (_entities[i] != null && _entities[i].ShouldReap())
			{
		...
		if (gameState == GameStates.Ingame)
		{
			...
			{
				int i = 0;
				while (i < checkpointPositions.Count && player.position.X >= (float)(checkpointPositions[i] * tileSize))
				{
					foreach (Entity entity2 in _entities)
					{
						if (entity2 is CheckpointIndicator indicator && (int)(indicator.position.X / (float)tileSize) == checkpointPositions[i])
						{
							indicator.Activate();
							break;
						}
					}
					checkpointPosition = ((float)checkpointPositions[i] + 0.5f) * (float)tileSize;
					ReapEntities();
	...
	public void ShowMap()
	{
		...
		else if (currentTheme == 3)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				AddEntity(new MineDebris(new Rectangle(51, 53, 9, 9), new Vector2(250f, 136f) + new Vector2(Game1.random.Next(-20, 31), Game1.random.Next(-20, 21)), Game1.random.Next(-30, 31), Game1.random.Next(-70, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, holdLastFrame: false, 0.8f + 0.01f * (float)i));
			}
		}
		...
		else if (currentTheme == 5)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				AddEntity(new MineDebris(new Rectangle(427, 607, 6, 6), new Vector2(205 + Game1.random.Next(3, 14), 26 + Game1.random.Next(6, 14)), Game1.random.Next(-30, 31), Game1.random.Next(-60, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, holdLastFrame: false, 5f + (float)i * 0.005f));
			}
		}
		if (currentTheme == 9)
		{
			for (int i = 0; i < 8; i++)
			{
				AddEntity(new MineDebris(new Rectangle(368, 784, 16, 16), new Vector2(274 + Game1.random.Next(-19, 20), 46 + Game1.random.Next(6, 14)), Game1.random.Next(-4, 5), -16f, 0f, 0.05f, 2f, 1f, 3, 0.33f, 0.35f, holdLastFrame: true, 1f + (float)i * 0.1f)).SetStartSound("dirtyHit");
			}
		}
	...
	public void draw(SpriteBatch b)
	{
		...
		if (gameState != GameStates.Map)
		{
			...
			else if (gameState != GameStates.Cutscene)
			{
				...
				for (int i = 0; i < lakeDecor.Count; i++)
				{
					lakeDecor[i].Draw(b);
				}
				for (int i = 0; i <= screenWidth / backBGSource.Width + 2; i++)
				{
					b.Draw(texture, TransformDraw(new Vector2(0f - backBGPosition + (float)(i * backBGSource.Width), backBGYOffset)), backBGSource, backBGTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.7f);
				}
				for (int i = 0; i < screenWidth / midBGSource.Width + 2; i++)
				{
					b.Draw(texture, TransformDraw(new Vector2(0f - midBGPosition + (float)(i * midBGSource.Width), 162 - midBGSource.Height + midBGYOffset)), midBGSource, midBGTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.6f);
				}
			}
		}
		foreach (Entity entity in _entities)
		...
		if (gameState == GameStates.Title)
		{
			...
			if (gameMode == 2)
			{
				...
				for (int i = 0; i < 5 && i < _currentHighScores.Count; i++)
				{
					...
					KeyValuePair<string, int> score = _currentHighScores[i];
					int score_text_width = (int)Game1.dialogueFont.MeasureString(score.Value.ToString() ?? "").X / 4;
					b.DrawString(Game1.dialogueFont, "#" + (i + 1), TransformDraw(draw_position), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
					...
					b.DrawString(Game1.dialogueFont, "#" + (i + 1), TransformDraw(draw_position + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
		...
		else if (gameState != GameStates.FruitsSummary)
		{
			...
			{
				for (int j = 0; j < waterfallWidth; j += 2)
				{
					for (int i = -2; i <= screenHeight / tileSize + 1; i++)
					{
						b.Draw(texture, TransformDraw(new Vector2((float)(screenWidth + tileSize * j) - waterFallPosition, i * tileSize + (int)(_totalTime * 48.0 + (double)(tileSize * 100)) % tileSize)), new Rectangle(48, 32, 16, 16), waterfallTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.2f);
					}
				}
			}
		}
		...
		{
			_shakeOffset = Vector2.Zero;
			Vector2 draw_position = new Vector2(4f, 4f);
			if (gameMode == 2)
			{
				string txtbestScore = Game1.content.LoadString("Strings\\StringsFromCSFiles:MineCart.cs.12115");
				b.DrawString(Game1.dialogueFont, Game1.content.LoadString("Strings\\StringsFromCSFiles:FishingGame.cs.10444", this.score), TransformDraw(draw_position), Color.White, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1f);
				b.DrawString(Game1.dialogueFont, Game1.content.LoadString("Strings\\StringsFromCSFiles:FishingGame.cs.10444", this.score), TransformDraw(draw_position + new Vector2(1f, 1f)), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.11f);
				draw_position.Y += 10f;
				b.DrawString(Game1.dialogueFont, txtbestScore + currentHighScore, TransformDraw(draw_position), Color.White, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1f);
				b.DrawString(Game1.dialogueFont, txtbestScore + currentHighScore, TransformDraw(draw_position + new Vector2(1f, 1f)), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.11f);
			}
			else
			{
				draw_position.X = 4f;
				...
				{
					b.Draw(texture, TransformDraw(draw_position), new Rectangle(160, 32, 16, 16), Color.White, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.07f);
					b.Draw(texture, TransformDraw(draw_position + new Vector2(1f, 1f)), new Rectangle(160, 32, 16, 16), Color.Black, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.071f);
					draw_position.X += 18f;
					if (draw_position.X > 90f && i < livesLeft - 1)
					{
						draw_position.X = 4f;
						draw_position.Y += 18f;
					}
				...
				{
					b.Draw(texture, TransformDraw(draw_position), new Rectangle(160, 48, 16, 16), Color.White, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.07f);
					b.Draw(texture, TransformDraw(draw_position + new Vector2(1f, 1f)), new Rectangle(160, 48, 16, 16), Color.Black, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.071f);
					draw_position.X -= 18f;
				}
			}
			draw_position.X = 4f;
			...
			for (int i = 0; i < 3; i++)
			{
				...
				if (currentFruitCheckMagnitude > 0f && i == currentFruitCheckIndex - 1)
				{
				...
				if (_collectedFruit.Contains((CollectableFruits)i))
				{
					b.Draw(texture, TransformDraw(draw_position + shake_magnitude), new Rectangle(160 + i * 16, 0, 16, 16), Color.White, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.07f);
					b.Draw(texture, TransformDraw(draw_position + new Vector2(1f, 1f) + shake_magnitude), new Rectangle(160 + i * 16, 0, 16, 16), Color.Black, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.075f);
				}
				else
				{
					b.Draw(texture, TransformDraw(draw_position + shake_magnitude), new Rectangle(160 + i * 16, 16, 16, 16), Color.White, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.07f);
					b.Draw(texture, TransformDraw(draw_position + shake_magnitude + new Vector2(1f, 1f)), new Rectangle(160 + i * 16, 16, 16, 16), Color.Black, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.075f);
				}
				draw_position.X += 18f;
			}
			if (gameMode == 3)
			{
				draw_position.X = 4f;
				draw_position.Y += 18f;
				b.Draw(texture, TransformDraw(draw_position), new Rectangle(0, 272, 9, 11), Color.White, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.07f);
				b.Draw(texture, TransformDraw(draw_position + new Vector2(1f, 1f)), new Rectangle(0, 272, 9, 11), Color.Black, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.08f);
				draw_position.X += 12f;
				b.DrawString(Game1.dialogueFont, coinCount.ToString("00"), TransformDraw(draw_position), Color.White, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.01f);
				b.DrawString(Game1.dialogueFont, coinCount.ToString("00"), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-3f, -3f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, coinCount.ToString("00"), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-2f, -2f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, coinCount.ToString("00"), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-1f, -1f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, coinCount.ToString("00"), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-3.5f, -3.5f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, coinCount.ToString("00"), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-1.5f, -1.5f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, coinCount.ToString("00"), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-2.5f, -2.5f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
			}
			if (Game1.IsMultiplayer)
			{
				string time_of_day_string = Game1.getTimeOfDayString(Game1.timeOfDay);
				draw_position = new Vector2((float)screenWidth - Game1.dialogueFont.MeasureString(time_of_day_string).X / 4f - 4f, 4f);
				Color timeColor = Color.White;
				b.DrawString(Game1.dialogueFont, Game1.getTimeOfDayString(Game1.timeOfDay), TransformDraw(draw_position), timeColor, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.01f);
				b.DrawString(Game1.dialogueFont, Game1.getTimeOfDayString(Game1.timeOfDay), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-3f, -3f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, Game1.getTimeOfDayString(Game1.timeOfDay), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-2f, -2f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, Game1.getTimeOfDayString(Game1.timeOfDay), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-1f, -1f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, Game1.getTimeOfDayString(Game1.timeOfDay), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-3.5f, -3.5f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, Game1.getTimeOfDayString(Game1.timeOfDay), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-1.5f, -1.5f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
				b.DrawString(Game1.dialogueFont, Game1.getTimeOfDayString(Game1.timeOfDay), TransformDraw(draw_position + new Vector2(1f, 1f)) + new Vector2(-2.5f, -2.5f), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.02f);
			}
			if (gameState == GameStates.Ingame)
			{
				float left_edge = (float)(screenWidth - 192) / 2f;
				float right_edge = left_edge + 192f;
				draw_position = new Vector2(left_edge, 4f);
				for (int i = 0; i < 12; i++)
				{
					...
					b.Draw(texture, TransformDraw(draw_position), source_rect, Color.White, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.15f);
					b.Draw(texture, TransformDraw(draw_position + new Vector2(1f, 1f)), source_rect, Color.Black, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.17f);
					draw_position.X += 16f;
				}
				b.Draw(texture, TransformDraw(draw_position), new Rectangle(176, 64, 16, 16), Color.White, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.15f);
				draw_position.X += 8f;
				string level_text = (levelsBeat + 1).ToString() ?? "";
				draw_position.Y += 3f;
				b.DrawString(Game1.dialogueFont, level_text, TransformDraw(draw_position - new Vector2(Game1.dialogueFont.MeasureString(level_text).X / 2f / 4f, 0f)), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1f);
				draw_position.X += 1f;
				draw_position.Y += 1f;
				draw_position = new Vector2(left_edge, 4f);
				if (player != null && player.visible)
				{
					draw_position.X = Utility.Lerp(left_edge, right_edge, Math.Min(player.position.X / (float)(distanceToTravel * tileSize), 1f));
				}
				b.Draw(texture, TransformDraw(draw_position), new Rectangle(240, 48, 16, 16), Color.White, 0f, new Vector2(8f, 0f), GetPixelScale(), SpriteEffects.None, 0.12f);
				b.Draw(texture, TransformDraw(draw_position + new Vector2(1f, 1f)), new Rectangle(240, 48, 16, 16), Color.Black, 0f, new Vector2(8f, 0f), GetPixelScale(), SpriteEffects.None, 0.13f);
				if (checkpointPosition > (float)tileSize * 0.5f)
				{
					draw_position.X = Utility.Lerp(left_edge, right_edge, checkpointPosition / (float)(distanceToTravel * tileSize));
					b.Draw(texture, TransformDraw(draw_position), new Rectangle(224, 48, 16, 16), Color.White, 0f, new Vector2(8f, 0f), GetPixelScale(), SpriteEffects.None, 0.125f);
					b.Draw(texture, TransformDraw(draw_position + new Vector2(1f, 1f)), new Rectangle(224, 48, 16, 16), Color.Black, 0f, new Vector2(8f, 0f), GetPixelScale(), SpriteEffects.None, 0.135f);
				}
			}
		}
