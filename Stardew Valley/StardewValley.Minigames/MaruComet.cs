...
public class MaruComet : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		for (int i = flybysClose.Count - 1; i >= 0; i--)
		{
			flybysClose[i] = new Vector2(flybysClose[i].X + 0.8f * (float)time.ElapsedGameTime.Milliseconds * 1.5f, flybysClose[i].Y - 0.8f * (float)time.ElapsedGameTime.Milliseconds * 1.5f);
			if (cometColorOrigin.Y + flybysClose[i].Y < centerOfScreen.Y - (float)(143 * zoom / 2))
			{
				flybysClose.RemoveAt(i);
			}
		}
		for (int i = flybysFar.Count - 1; i >= 0; i--)
		{
			flybysFar[i] = new Vector2(flybysFar[i].X + 0.8f * (float)time.ElapsedGameTime.Milliseconds * 0.5f, flybysFar[i].Y - 0.8f * (float)time.ElapsedGameTime.Milliseconds * 0.5f);
			if (cometColorOrigin.Y + flybysFar[i].Y < centerOfScreen.Y - (float)(143 * zoom / 2))
			{
				flybysFar.RemoveAt(i);
			}
		}
	...
	public void draw(SpriteBatch b)
	{
		...
		foreach (Vector2 v in flybysClose)
		{
			b.Draw(cometTexture, cometColorOrigin + v, new Rectangle(0, 0, 8, 8), Color.White * 0.4f, 0f, Vector2.Zero, zoom + 1, SpriteEffects.None, 0.24f);
		}
		foreach (Vector2 v in flybysFar)
		{
			b.Draw(cometTexture, cometColorOrigin + v, new Rectangle(0, 0, 8, 8), Color.White * 0.4f, 0f, Vector2.Zero, zoom - 1, SpriteEffects.None, 0.24f);
		}
